...
def infer(args):
    ...
    with open(results_file, 'a', encoding='utf-8') as f:
        ...
        if pass_at_k_list:
            ...
            print(f"Maj@{args.n_sampling}: {correct_cnt}/{len(examples)} = {correct_cnt / len(examples):.4f}")
            f.write(f"Maj@{args.n_sampling}: {correct_cnt}/{len(examples)} = {correct_cnt / len(examples):.4f}\n")
        f.flush()
