...
def parse_args():
    ...
    parser.add_argument('--data_name',
                        type=str,
                        default="math",
                        help='identify how to extract answer')
    ...
    parser.add_argument('--end_idx',
                        type=int,
                        default=-1,
                        help="data[start:end], if -1, data[start:]")
...
def infer(args):
    ...
    if os.path.exists(out_file):
        print(
            f"Completely same name file({out_file}) exist, skip generation, save file and check correct"
        )
        return
    os.makedirs(f'{args.output_dir}/{model_name}/{args.data_name}',
                exist_ok=True)
    ...
    for example in tqdm(examples, total=len(examples)):
        ...
        if args.use_few_shot:
            cur_prompt = few_shot_prompt + question_format.format(
                question=question)
        else:
    ...
    for i in tqdm(range(len(examples)), "check correct..."):
        ...
        generated_answers = [
            extract_answer(generated_response, args.data_name)
            for generated_response in generated_responses
        ]
        is_correct_list = [
            check_is_correct(generated_answer, gt_ans)
...
if __name__ == "__main__":
    ...
    args.n_sampling = 256  # Maj@k
    infer(args)

    # test_majk(data_path="/home/aiops/chenxw/math-eval/outputs/Qwen/Qwen2.5-7B/aime/test_qwen-instruct_t0.6_k8_s0_e30.jsonl", k=8)
